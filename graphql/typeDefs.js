...
module.exports = gql`
  ...
  type Query {
    ...
    getFriendRequests(userId: String!): [User]! # Get an array of user objects who the given user has been sent friend requests by
  ...
  # Mutations modify information stored in MongoDB
  type Mutation {
    signupUser(
      name: String!
      email: String!
    ...
    addEventRecordingUrl(
      ...
      userId: String!
      finish: Boolean!
    ): [String] # Add URL to the latest EventRecording object
    ...
    detectDanger(recordingBytes: String, userId: String!): String! # Indicates whether danger is detected; returns strings "start" to trigger recording if needed, otherwise "stop"
    handleDanger(
      recordingBytes: String
      userId: String!
      eventRecordingUrl: String
    ): String! # Checks whether or not to stop recording after it has been started; returns strings "stop" or "panic" if needed, otherwise "start"
    matchStartTranscription(transcription: String!, userId: String!): String! # Matches keywords to start recording (start key, police/thief tokens). Returns "start" if detected, otherwise maintains "stop"
    matchStopTranscription(transcription: String!, userId: String!): String! # Matches keywords to stop recording (stop/panic key). Returns "stop" or "panic" if detected, otherwise maintains "start"
    ...
    addFriend(userId: String!, requesterId: String!): [String] # Adds requesterId User to userId User's friend list, and vice versa ONLY if there was a request made
    removeFriend(userId: String!, friendId: String!): [String] # Removes userId User from friendId User's friend list and vice versa only if they are friends
    setMessageInfo(
      userId: String!
    ...
    sendTwilioSMS(phoneNumber: String!, message: String!): String! # Sends an SMS message to pre-set number using Twilio, returns the message sent as a string
    addToFinishedEventRecording(
      eventRecordingUrl: String!
      userId: String!
    ): [String] # Adds an eventRecordingUrl to a finished or unfinished eventRecording object, returns updated eventRecordingUrls of eventRecording object
